...
from utils.kde_estimation_mi import KDE_IXT_estimation
...
def train(hyp, opt, device, tb_writer=None):
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        logger.info(('\n' + '%10s' * 9) % ('Epoch', 'gpu_mem', 'GIoU', 'obj', 'cls', 'TOTAL', 'I_XT', 'targets', 'img_size'))
        if rank in [-1, 0]:
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                ...
                # 加入信息瓶颈IB
                beta17, beta20, beta23 = 0.00025, 0.00025, 0.0001
                # beta17, beta20, beta23 = 0.0, 0.0, 0.0
                ib_variables17, ib_variables20, ib_variables23 = model.get_ib_variables()
                ib_variables17 = ib_variables17.view(ib_variables17.shape[0], -1)  # n_batch, n_channel * W * H
                ib_variables20 = ib_variables20.view(ib_variables20.shape[0], -1)  # n_batch, n_channel * W * H
                ib_variables23 = ib_variables23.view(ib_variables23.shape[0], -1)  # n_batch, n_channel * W * H
                logvat_t = torch.Tensor([1.0]).to(device)
                I_XT17 = KDE_IXT_estimation(logvar_t=logvat_t, mean_t=ib_variables17)
                I_XT20 = KDE_IXT_estimation(logvar_t=logvat_t, mean_t=ib_variables20)
                I_XT23 = KDE_IXT_estimation(logvar_t=logvat_t, mean_t=ib_variables23)
                I_XT_loss = (I_XT17 * beta17 + I_XT20 * beta20 + I_XT23 * beta23) / torch.log(torch.tensor(2.0))   # in bits
            ...
            if rank in [-1, 0]:
                ...
                s = ('%10s' * 2 + '%10.4g' * 6 + '%10.4g') % (
                    '%g/%g' % (epoch, epochs - 1), mem, *mloss, I_XT_loss, targets.shape[0], imgs.shape[-1])
        ...
        if rank in [-1, 0]:
            ...
            if save:
                ...
                sub_str = "_e{}.".format(epoch)
                new_last = str(last).replace(".", sub_str)
                shutil.copy(last, new_last)
                if best_fitness == fi:
