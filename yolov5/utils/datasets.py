...
class LoadImages:  # for inference
    def __init__(self, path, img_size=640):
        ...
        images = [x for x in files if (os.path.splitext(x)[-1].lower() in img_formats) and ('mask' not in x)]
    ...
    def __next__(self):
        ...
        ww = img0.shape[0]
        if ww != 640:
            img0 = np.rot90(img0,axes=(0,1))
            img0 = cv2.flip(img0, 2)
        # Padded resize
        # print('^^^^^^^^^^^^^^^^^^^^^^^^^^^',img0.shape)
    ...
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        if not isinstance(path, list):
            ...
            for sub_path in sub_paths:
                if os.path.isdir(os.path.join(path, sub_path)):
                    path_list.append(os.path.join(path, sub_path))
        path = path_list
        ...
        try:
            ...
            self.img_files = sorted(
                [x.replace('/', os.sep) for x in f if (os.path.splitext(x)[-1].lower() in img_formats) and ('mask' not in x) and ('mixed' not in x)])
