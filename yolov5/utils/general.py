...
def plot_one_classification(x, img, color=None, label=None, line_thickness=None, label_on_top=True):
    ...
    if label:
        ...
        if label_on_top:
            ...
            # cv2.putText(img, class_id, (c1[0], c1[1] + t_size[1]), 0, tl / 3, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
        else:
            c2_ = c1[0] + t_size[0], c2[1] + t_size[1] + 3
            cv2.rectangle(img, (c1[0], c2[1]), c2_, color, -1, cv2.LINE_AA)  # filled
            cv2.putText(img, label, (c1[0], c2[1] + t_size[1]), 0, tl / 3, [255, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
            # cv2.putText(img, class_id, (c1[0], c1[1] - 2), 0, tl / 3, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)


def plot_one_box(x, img, color=None, label=None, line_thickness=None, label_on_top=True):
    # Plots one bounding box on image img
    tl = line_thickness or round(0.002 * (img.shape[0] + img.shape[1]) / 2) + 1  # line/font thickness
    color = color or [random.randint(0, 255) for _ in range(3)]
    c1, c2 = (int(x[0]), int(x[1])), (int(x[2]), int(x[3]))
