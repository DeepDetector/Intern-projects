...
class CrowdHuman(torch.utils.data.Dataset):
    ...
    def load_record(self, record):
        if self.training:
            ...
            if_flap = False
        # image
        image_path = os.path.join(record['ID']+'.jpg')
        # print(image_path)
        ...
        if self.training:
            ...
            # print(gtboxes[0])
            # cv2.circle(image, (int(gtboxes[0][0]),int(gtboxes[0][1])), 4, color=(0, 255, 0))
            # cv2.circle(image, (int(gtboxes[0][0]+gtboxes[0][2]),int(gtboxes[0][1]+gtboxes[0][3])), 4, color=(0, 255, 255))
