...
savedir = '/raid/huayan_nfs/share/data/detection/generate_data_iou0.05_0.1/'
minthr = 0.05 # 设定iou分割阈值
maxthr = 0.1
def iou(box1, box2):
        ...
        # 如果图片有多个框框，判断框框是否有交集
            ...
            for i in range(num):
                ...
                for j in range(num):
                    if j==i:
                        continue
                    iouscore = is2anchorcross(objects[i],objects[j])
                    if iouscore>minthr and iouscore<maxthr:
                        isclass0 = False
                        break
                    elif iouscore > maxthr:
...
# 生成分类数据，一个框如果和别的框没有交集为0，否则为1,iou阈值为一个范围
